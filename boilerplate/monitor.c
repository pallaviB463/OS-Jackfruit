 ...
 *
 * All six TODOs implemented.
 *
 * Synchronization choice: mutex (not spinlock).
 * Both code paths that access the list – the ioctl handler and the timer
 * callback – may sleep (kmalloc with GFP_KERNEL, copy_from_user).
 * Spinlocks cannot be held across sleeping operations, so a mutex is the
...
struct monitor_entry {
    ...
    int              soft_warned;   /* 1 once the one-time warning has fired */
    struct list_head list;
};

/* ---------------------------------------------------------------
 * TODO 2 DONE: global list + lock
 * Mutex chosen: ioctl path calls copy_from_user and kmalloc(GFP_KERNEL),
 * both of which may sleep – spinlocks are forbidden in that context.
 * --------------------------------------------------------------- */
...
/* ---------------------------------------------------------------
 * Provided: hard-limit event helper (kills the process)
 * --------------------------------------------------------------- */
...
/* ---------------------------------------------------------------
 * TODO 3 DONE: periodic monitoring timer callback
 * --------------------------------------------------------------- */
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitor_list, list) {
        ...
        /* Process has exited – remove silently */
        if (rss < 0) {
            printk(KERN_INFO
                   "[container_monitor] pid=%d (container=%s) gone, "
                   "removing entry\n",
        ...
        if (!entry->soft_warned &&
            (unsigned long)rss > entry->soft_limit) {
         ...
         * list_for_each_entry_safe already saved 'tmp', so freeing
         * 'entry' here is safe. */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    /* ---------------------------------------------------------------
     * TODO 4 DONE: register a new entry
     * --------------------------------------------------------------- */
    ...
    /* ---------------------------------------------------------------
     * TODO 5 DONE: unregister an entry by PID
     * --------------------------------------------------------------- */
    {
        struct monitor_entry *entry, *tmp;
        int found = 0;
        ...
        list_for_each_entry_safe(entry, tmp, &monitor_list, list) {
            if (entry->pid == req.pid) {
                printk(KERN_INFO
                       "[container_monitor] Unregistered container=%s "
                       "pid=%d\n",
                       entry->container_id, entry->pid);
                list_del(&entry->list);
                kfree(entry);
...
{
    /* ---------------------------------------------------------------
     * TODO 6 DONE: free all remaining entries on unload.
     * timer_delete_sync must come first to ensure the timer callback is
     * not running concurrently when we free the list.
     * --------------------------------------------------------------- */
    struct monitor_entry *entry, *tmp;
